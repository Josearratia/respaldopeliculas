...
if (!file_exists('wp-content/cache/googledrive')) {
    mkdir('wp-content/cache/googledrive', 0777, true);
}
...
function Drive($link) {
	...
	if(file_exists('wp-content/cache/googledrive/'.$file_name.'.cache')) {
		$fopen = file_get_contents('wp-content/cache/googledrive/'.$file_name.'.cache');
		$data  = explode('@@', $fopen);
...
function gd_cache($link, $source) {
	...
	$file      = fopen("wp-content/cache/googledrive/".$file_name.".cache",'w');
	fwrite($file,$string);
	fclose($file);
