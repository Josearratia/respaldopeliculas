	...
	function dt_metabox_episodes_display( $post) {
	    ...
	    $options = array(
	        ...
	        array(
	            ...
	            'prelink' => 'https://image.tmdb.org/t/p/w780',
	            'ajax'    => false
	        ),
...
if( ! function_exists( 'dt_metabox_episodes_save' ) ) {
	function dt_metabox_episodes_save( $post_id ) {
		...
		// Insert String for episodes LINKs
		$stringmeta		= get_post_meta( $post_id, 'dt_string' );
		$stringidtm		= isset( $_POST['ids'] ) ? $_POST['ids'] : null;
		$stringkey		= 'tv'.dt_dbmovies_key_string(6). $stringidtm;
		if ( empty( $stringmeta ) ) update_post_meta( $post_id, 'dt_string', $stringkey );
		...
		$backdropurl = isset( $_POST['dt_backdrop'] ) ? 'https://image.tmdb.org/t/p/w342'. $_POST['dt_backdrop'] : null;
