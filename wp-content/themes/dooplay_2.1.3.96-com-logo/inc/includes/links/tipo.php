...
if( ! function_exists( 'doo_links' ) ) {
	...
	add_action('init', 'doo_links', 0 );
	get_template_part('inc/includes/links/metabox');
}

if( ! function_exists( 'link_table_head' ) ) {
...
if( ! function_exists( 'link_table_content' ) ) {
	function link_table_content( $column_name, $post_id ) {
		...
		$type = get_post_meta( $post_id, 'links_type', true );
	    $url = get_post_meta( $post_id, 'links_url', true );
			echo '<img src="', DOO_GICO,  ($type == 'Torrent') ? 'https://utorrent.com' : saca_dominio($url) ,'"> &nbsp;',  ($type == 'Torrent') ? 'Torrent' : saca_dominio($url);
	    }
...
if( ! function_exists( 'add_pending_count_filter_link' ) ) {
...
if( ! function_exists( 'esc_attr_restore_link' ) ) {
...
if( ! function_exists( 'remove_esc_attr_and_count_link' ) ) {
