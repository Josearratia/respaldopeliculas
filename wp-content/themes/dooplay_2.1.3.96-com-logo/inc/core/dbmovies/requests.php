...
function dt_dbmovies_request_search_content() {
	...
	if( is_user_logged_in() AND wp_verify_nonce($nonce, 'dbmovies_requests_users') ) {
...
function dt_dbmovies_request_post_content() {
	...
	$nonce 	= isset( $_REQUEST['nonce'] )	? $_REQUEST['nonce'] 	: null;
	// Conditional
	if( is_user_logged_in() AND wp_verify_nonce($nonce, $id. '_post_request') ) {
...
function dt_dbmovies_post_archive() {
	...
	if( $id != null ) {
		...
		echo ( $backdrop != null ) ? "<div class='backdrop'><img src='https:\/\/image.tmdb.org/t/p/w780{$backdrop}'> <span>{$maintype}</span></div>" : null;
		echo "<div class='data'>";
		echo "<h3>{$title}</h3>";
