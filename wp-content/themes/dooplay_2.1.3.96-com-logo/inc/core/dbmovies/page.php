...
<div class="wrap dbmovies" id="dbmovies-page">
	...
	<div id="dbmovies_settings" class="settbox jump">
		<form id="dbmovies-save" data-action="settings">
			<fieldset class="pro perico">
				<label id="cprt" for="dbmv"><?php _d('Dbmovies Options'); if($popt01 == 'valid' AND $popt10 == 'red') { echo '<a id="activate_dbmovies">'. __d('Get license key now') .'</a>'; } ?></label>
				<input type="text" id="dbmv" name="dbmv" placeholder="<?php _d('Dbmovies API key'); ?>" value="<?php echo $popt04; ?>" class="<?php echo $popt10; ?>">
				<?php if($popt04) { echo '<p id="pnote">'. __d('<strong>NOTE:</strong> Do not share this information, this key is only authorized for your website.') .'</p>'; } ?>
			</fieldset>
	...
	<div class="toolbar dual">
		<form id="dbmovies-filter" data-action="filter">
			...
			<div class="item">
				<input id="import_all" data-next="1" class="button button-primary" type="button" value="<?php _d('Import all'); ?>" style="display:none">
			</div>
		</form>
