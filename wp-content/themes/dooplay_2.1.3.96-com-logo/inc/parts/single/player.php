...
<div id="playex" class="player_sist <?php if(get_cookie('dt_player_width') == 'full') { echo 'fullplayer'; } ?>">
	...
	<div class="control">
		<nav class="player">
			<ul class="options">
				<li>
					<a class="sources"><i class="icon-menu listsormenu"></i> <b><?php _d('option'); ?></b></a>				
					<?php  if ( $dt_player ) : ?>
						<ul class="idTabs sourceslist">
						<?php $numerado = 1; { foreach ( $dt_player as $field ) { ?>
							<li><a class="options" href="#option-<?php echo $numerado; ?>">
							<b class="icon-play_arrow"></b> <?php echo $field['name']; ?> 
							<?php if($field['idioma']) { ?><span class="dt_flag"><img src="<?php echo DOO_URI, '/assets/img/flags/',$field['idioma'],'.png'; ?>"></span><?php } ?>
							</a></li>
						<?php $numerado++; } } ?> 
						</ul>
					<?php endif; ?>
