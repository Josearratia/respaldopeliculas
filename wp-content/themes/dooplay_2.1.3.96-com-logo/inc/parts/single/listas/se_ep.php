...
<div class="se-c">
	<div class="se-q">
		...
		<span class="title"><?php _d('Season'); ?>  <?php echo $value_t['season']; ?> <i><?php $date = new DateTime(data_of('air_date', $value_t['id'])); echo $date->format(DOO_TIME); ?></i>
	...
	<?php foreach($episodes as $key_c=>$value_c) { if($value_t['season'] == $value_c['season']) { ?>
			<li>
				<div class="imagen"><a href="<?php echo get_permalink( $value_c['id'] ); if(data_of('repeatable_fields', $value_c['id'])) { echo ''; } ?>"><img src="<?php if($thumb_id = get_post_thumbnail_id($value_c['id'])) { $thumb_url = wp_get_attachment_image_src($thumb_id,'dt_episode_a', true); echo $thumb_url[0]; } else { dt_image('dt_backdrop', $value_c['id'], 'w154'); } ?>"></a></div>
				...
				<div class="episodiotitle">
					...
					<span class="date"><?php $date = new DateTime(data_of('air_date', $value_c['id'])); echo $date->format(DOO_TIME); ?></span>
				</div>
			</li>
	<?php } } ?>
	</ul>
